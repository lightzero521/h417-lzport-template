...
#define TEST_SERIAL_PORT       1U //USART2
...
void lzport_test_user(void)
{
    ...
    lzport_gpio_mode_output(LZPORT_GPIO_C, LZPORT_GPIO_PIN_2, LZPORT_GPIO_SPEED_LOW, LZPORT_GPIO_OPEN_DRAIN);
    lzport_gpio_mode_output(LZPORT_GPIO_C, LZPORT_GPIO_PIN_3, LZPORT_GPIO_SPEED_LOW, LZPORT_GPIO_OPEN_DRAIN);
    lzport_gpio_reset(LZPORT_GPIO_C, LZPORT_GPIO_PIN_2);
    lzport_gpio_reset(LZPORT_GPIO_C, LZPORT_GPIO_PIN_3);
    Delay_Ms(300);
    lzport_gpio_set(LZPORT_GPIO_C, LZPORT_GPIO_PIN_2);
    lzport_gpio_set(LZPORT_GPIO_C, LZPORT_GPIO_PIN_3);

    rtc_status = lzport_rtc_init();
    printf("lzport-test-user: rtc %s (%d)\r\n",
           (rtc_status == LZPORT_OK) ? "ok" : "failed", (int)rtc_status);
...
void lzport_test_runner(void)
{
    if (g_echo_state == TEST_RX_START) {
        ...
        if (lzport_serial_read_async(TEST_SERIAL_PORT, g_echo_buffer, sizeof(g_echo_buffer), rx_done, 0) != LZPORT_OK) {
            g_echo_state = TEST_RX_START;
        }
    } else if (g_echo_state == TEST_TX_START) {
        g_echo_state = TEST_TX_BUSY;
        if (lzport_serial_write_async(TEST_SERIAL_PORT, g_echo_buffer, g_echo_length, tx_done, 0) != LZPORT_OK) {
            g_echo_state = TEST_RX_START;
        }
    }
}
